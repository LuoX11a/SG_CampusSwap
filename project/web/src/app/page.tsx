...
export default function HomePage() {
  const { items, total, hasNext, isLoading, error, fetchItems, appendItems } = useItemStore();
  const { filters, setCategory } = useFilterStore();
  const sentinelRef = useRef<HTMLDivElement>(null);
  const [hasLoaded, setHasLoaded] = useState(false);

  useEffect(() => {
    fetchItems(filters).finally(() => setHasLoaded(true));
  }, [filters]);
    ...
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">
          {hasLoaded
            ? `${total} items available near your campus`
            : 'Loading items...'}
        </p>
      </div>
      ...
      {error && items.length === 0 && (
        <div className="text-center py-20">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">Failed to load items</h2>
      ...
      {/* Initial Loading — show skeleton cards before first data arrives */}
      {!hasLoaded && !error && (
      ...
      {/* Empty State — only after first load and truly no items */}
      ...
      {/* Items Grid — show when data is available */}
